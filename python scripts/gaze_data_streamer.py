...
Heatmap_pyt_streamer = UDPSender('127.0.0.1',6501)
gazepoint_streamer = UDPSender('127.0.0.1',6502)
print ('gaze streaming initialised!')

ctx = zmq.Context()

req = zmq_receiver.init_requester(ctx,50021)
...
while True :
    ...
    if g_data['confidence'] > 0.6 :
        ...
        gazepoint_streamer.send('%s,%s'%(str(norm_pos[0]),str(norm_pos[1])))
